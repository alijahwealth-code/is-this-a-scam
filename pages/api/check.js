...
module.exports = async function handler(req, res) {
  ...
  const prompt = `You are a cybersecurity and scam detection expert. Analyze the following content and determine if it is a scam, phishing attempt, fraud, or suspicious.
...
Respond in valid JSON only, no markdown, with this exact structure:
{
  ...
  "how_it_works": "<paragraph explaining the scam tactic if applicable>",
  "what_to_do": "<short actionable advice>"
}`;
  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://isthisascam.vercel.app',
      },
      body: JSON.stringify({
